...
public class MainActivity extends MetaioCloudPluginBaseActivity implements DialogInterface.OnDismissListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		MetaioPlugin.isDebuggable = BuildConfig.DEBUG;

		// TODO Set authentication if a private channel is used
		// MetaioPlugin.setAuthentication("username", "password");
        ...
        try {
            ...
            Utils.showErrorForCloudPluginResult(MetaioPlugin.ERROR_CPU_NOT_SUPPORTED, this);
            return;
        }
		int result = MetaioPlugin.initialize(null, getApplicationContext());
		...
		mMetaioPluginManager.initARELWebView(mGUIView);

		// Used to resume the camera preview
		mMetaioPluginManager.mIsInLiveMode = true;

		if (result!=MetaioPlugin.SUCCESS)
	...
	protected void onStart()
	{
		...
		if (channelID>0)
		{
			...
			mMetaioPluginManager.setChannel(channelID);
		}
	}
	...
	public void onScreenshot(Bitmap bitmap, boolean saveToGalleryWithoutDialog)
	{
		...
		{
			boolean result =
					MetaioCloudUtils.writeToFile(bitmap, CompressFormat.JPEG, 100, MetaioPlugin.mCacheDir,
							filename, false);
			...
			if (result)
			{
				...
				{
					final String path = new File(MetaioPlugin.mCacheDir, filename).getAbsolutePath();
	...
	@Override
	public ARMetaioPluginManager getARMetaioPluginManagerInstance()
	{
		if (mMetaioPluginManager==null)
			mMetaioPluginManager = new MyCloudPlugin(this);
		return mMetaioPluginManager;
	}
	...
	class MyCloudPlugin extends ARMetaioPluginManager
	{
		...
		public void onSurfaceChanged(int width, int height)
		{
			...
			mMetaioPluginManager.setRadarProperties(IGeometry.ANCHOR_TOP|IGeometry.ANCHOR_RIGHT, new Vector3d(
